...
qtd_marcas = df["brand"].value_counts().nlargest(10).sort_values(ascending=False)
qtd_marcas_df = qtd_marcas.reset_index()
qtd_marcas_df.columns = ["Marca", "Total Produtos"]

# Mostrar o gráfico de barras
fig_bar = px.bar(
    qtd_marcas_df.sort_values(by="Total Produtos", ascending=True),
    x="Total Produtos",
    y="Marca",
    orientation="h",
...
df = df[df["new_price"] > 0]  # Desconsiderando marcas com valores zerados
...
fig_price_bar = px.bar(
    top_10_average_prices_df.sort_values(by="Preço Médio", ascending=True),
    x="Preço Médio",
    y="Marcas",
    orientation="h",
    color_discrete_sequence=["#ffff00"],
...
st.subheader("Top 10 marcas com melhores avaliações até a página 15")
st.markdown("Distribuição média das avalições por marca")

# Avaliação média
df = df[
    df["reviews_rating_number"] > 0
]  # Desconsiderando marcas que não tiveram avaliação
avaliacao_marca = (
    df.groupby("brand")["reviews_rating_number"].mean().round(3).nlargest(10)
)
...
fig_avaliacoes_bar = px.bar(
    avaliacao_marca_df.sort_values(by="Avaliações Média", ascending=False),
    x="Marcas",
    y="Avaliações Média",
    orientation="v",
    color_discrete_sequence=["#ffff00"],
